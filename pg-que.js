...
module.exports =
    /**
     * [PgQueue description] This class contains operations that can be performed on queue data structure as a subscriber.
     */
    class PgQueue {
         ...
         * @param {*} writerPG Write connection for postgres
         * @param {*} pages Number of pages que should have for multiple subscribers.
         * @param {*} subscriber Subscriber to be used or created for this queue 
         */
        constructor(name, writerPG, pages = 20, subscriber = { "name": "Primary", "messagesPerBatch": 1 }) {
            ...
            this.tryDeleteSubscriber = this.tryDeleteSubscriber.bind(this);
                    ...
                    case -1: //First time install
                        for (let idx = 0; idx < this.#queries.Schema1.length; idx++) {
                            ...
                            let stepParams = {
                                ...
                                "subscriberstablename": "S-" + this.name,
        ...
        /**
         * [tryEnque description] Function to enqueue payloads into que
         *
         * @param   {object[]}  payloads  [payloads description] array of json objects that needs to be enqued, each element is item in queue.
         *
        ...
        async tryDeque(messageAcquiredTimeout = 3600, retry = 10) {
            ...
            while (retry > 0) {
                try {
                    message = await this.#writerPG.tx(transaction => {
        ...
        async tryAcknowledge(tokens, retry = 10) {
            ...
            return (message == undefined) ? undefined : message[0][this.#ackFunctionName];
        }

        /**
         * [tryDeleteSubscriber description]
         *
         * @return  {[type]}  [return description]
         */
        async tryDeleteSubscriber() {
            await this.#initialize(schemaVersion);

            let result = await this.#writerPG.tx(transaction => {
                return transaction.func(this.#deleteSubFunctionName, [this.#subscriber.name]);
            });

            return result[0][this.#deleteSubFunctionName];
        }
    }
